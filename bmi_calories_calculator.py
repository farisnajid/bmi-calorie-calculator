...
def meal_plan(calories):
    ...
    meal = {'breakfast': [('bread', 500), ('banana', 200)], 'lunch': [('baked salmon', 1200)], 'dinner':[('tuna sandwich', 1000)]}
...
def result_message():
    ...
    try:
        ...
        text = 'Your BMI is {}.\nYou should consume {} calories per day.\n\n{}'.format(bmi, format(calories,',d'), meal_plan_result(plan))
...
age_dropdown = ttk.Combobox(window, textvariable=age_entry,values=age_options, state = 'readonly', justify = tk.CENTER, width=21)
...
# height
height_entry = Entry(window, bg="white", width=11, justify = tk.CENTER)
height_entry.grid(row=5, column=1, columnspan = 2, sticky=W)
height_units = tk.StringVar()
height_units.set('m')
height_units_options = ttk.Combobox(window, textvariable=height_units,values=['cm', 'm'], state = 'readonly', justify = tk.CENTER, width=9)
height_units_options.grid(row=5, column=2,sticky=W)

# weight
weight_entry = Entry(window, bg="white", width=11, justify = tk.CENTER)
weight_entry.grid(row=6, column=1 ,columnspan = 2, sticky=W)
weight_units = tk.StringVar()
weight_units.set('kg')
weight_units_options = ttk.Combobox(window, textvariable=weight_units,values=['g', 'kg'], state = 'readonly', justify = tk.CENTER, width=9)
...
activity_dropdown = ttk.Combobox(window, textvariable=activity_entry,values=activity_options, state = 'readonly', justify = tk.CENTER, width=21)
...
Label(window, textvariable=result, justify="left", fg='blue').grid(row=11, column=0, columnspan = 3,sticky=W)

# buttons
Button(window, text="reset", width=8, command=reset).grid(row=9, column=1, sticky=W)
Button(window, text="calculate", width=8, command=result_message).grid(row=9, column=2, sticky=W)
Button(window, text="close", width=8, command=close_program).grid(row=9, column=0, sticky=W)
